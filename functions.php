...
add_filter('body_class', function ($body_class) {
    if (is_frame_mode() || is_publisher_mode()) {
    ...
    $content .=
        ...
        '&euro;</div>
            ...
            <div class="cta-wrapper">
                <a class="btn btn-primary btn-mdxl" href="' .
        $url .
        '">Jetzt bestellen</a>
            </div>
        </div>
...
function get_price_calculator($args = [])
{
    ...
    $content .=
        '
    <div id="priceCalculator" class="position-' .
        $position .
        '">
    ...
    <a href="' .
        $url .
        '" class="btn btn-primary btn-lg order-now">Jetzt beauftragen</a>
    </div>';
...
function get_countdown($args = []): string
{
    ...
    $content .=
        '
        <div class="countdown" data-countdown-end="' .
        $end .
        '" data-countdown>
            <div class="col">
...
/**
 * checks if publisher mode is required
 *
 * @param string $parameters - optional
 *
 * @return bool
 */
function is_publisher_mode(string ...$parameters): bool
{
    $is_publisher_mode = isset($_GET['p_id']);

    if (empty($parameters) && $is_publisher_mode) {
        return true;
    }

    foreach ($parameters as $parameter) {
        if (false === empty($parameter) && $_GET['p_id'] === $parameter) {
            return true;
        }
    }
